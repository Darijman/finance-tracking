...
export const AddNoteFormMobile = ({ financeCategories, setLastThreeUserNotes }: Props) => {
  const { user } = useAuth();
  ...
  return (
    <>
      ...
      <div className='addNote_page_wrapper'>
        <Form form={form} onFinish={onFinishHandler} onFinishFailed={onFinishFailedHandler} scrollToFirstError>
          <div className='addNoteMobile_form_container'>
            <div>
            ...
            <Form.Item name='category' rules={[{ required: true, message: '' }]}>
              <div className={`addNoteMobile_categories ${hasCategoryError ? 'error' : ''}`}>
